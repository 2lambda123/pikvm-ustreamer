...
static const struct {
  ...
  const unsigned format; // cppcheck-suppress unusedStructMember
    ...
    {"YUYV", V4L2_PIX_FMT_YUYV},   {"YVYU", V4L2_PIX_FMT_YVYU},
    {"UYVY", V4L2_PIX_FMT_UYVY},   {"RGB565", V4L2_PIX_FMT_RGB565},
    {"RGB24", V4L2_PIX_FMT_RGB24}, {"BGR24", V4L2_PIX_FMT_BGR24},
    {"MJPEG", V4L2_PIX_FMT_MJPEG}, {"JPEG", V4L2_PIX_FMT_JPEG},
};
...
static void _v4l2_buffer_copy(const struct v4l2_buffer *src,
                              struct v4l2_buffer *dest);
static bool _device_is_buffer_valid(us_device_s *dev,
...
static int _device_apply_resolution(us_device_s *dev, unsigned width,
                                    unsigned height);

static void _device_apply_controls(us_device_s *dev);
static int _device_query_control(us_device_s *dev, struct v4l2_queryctrl *query,
                                 const char *name, unsigned cid, bool quiet);
static void _device_set_control(us_device_s *dev,
                                const struct v4l2_queryctrl *query,
                                const char *name, unsigned cid, int value,
                                bool quiet);
...
int us_device_select(us_device_s *dev, bool *has_read, bool *has_write,
                     bool *has_error) {
  int retval;
  ...
  do {
    ...
    if (new_got) {
      if (new.index >= _RUN(n_bufs)) {
        US_LOG_ERROR("V4L2 error: grabbed invalid device buffer=%u, n_bufs=%u",
                     new.index, _RUN(n_bufs));
        return -1;
      }
...
#define FRAME_DATA(x_buf) _RUN(hw_bufs)[x_buf.index].raw.data
      ...
      if (GRABBED(new)) {
        US_LOG_ERROR("V4L2 error: grabbed device buffer=%u is already used",
                     new.index);
        return -1;
      }
      GRABBED(new) = true;
      ...
      if (broken) {
        US_LOG_DEBUG("Releasing device buffer=%u (broken frame) ...",
                     new.index);
        if (_D_XIOCTL(VIDIOC_QBUF, &new) < 0) {
          US_LOG_PERROR("Can't release device buffer=%u (broken frame)",
                        new.index);
          return -1;
        }
        GRABBED(new) = false;
      ...
      if (buf_got) {
        if (_D_XIOCTL(VIDIOC_QBUF, &buf) < 0) {
          US_LOG_PERROR("Can't release device buffer=%u (skipped frame)",
                        buf.index);
          return -1;
        }
        GRABBED(buf) = false;
...
#define HW(x_next) _RUN(hw_bufs)[buf.index].x_next
  ...
  HW(raw.grab_ts) = (long double)((buf.timestamp.tv_sec * (uint64_t)1000) +
                                  (buf.timestamp.tv_usec / 1000)) /
                    1000;
  US_LOG_DEBUG("Grabbed new frame: buffer=%u, bytesused=%u, grab_ts=%.3Lf, "
               "latency=%.3Lf, skipped=%u",
               buf.index, buf.bytesused, HW(raw.grab_ts),
               us_get_now_monotonic() - HW(raw.grab_ts), skipped);
#undef HW
...
static void _v4l2_buffer_copy(const struct v4l2_buffer *src,
  ...
  if (src->type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE) {
    ...
    memcpy(dest->m.planes, src->m.planes,
...
bool _device_is_buffer_valid(us_device_s *dev, const struct v4l2_buffer *buf,
  ...
  // frames. For example a VGA (640x480) webcam frame is normally >= 8kByte
  // large, corrupted frames are smaller.
  if (buf->bytesused < dev->min_frame_size) {
    US_LOG_DEBUG("Dropped too small frame, assuming it was broken: buffer=%u, "
                 "bytesused=%u",
  ...
  if (us_is_jpeg(dev->run->format)) {
    if (buf->bytesused < 125) {
      ...
      US_LOG_DEBUG("Discarding invalid frame, too small to be a valid JPEG: "
                   "bytesused=%u",
                   buf->bytesused);
      return false;
    }
    ...
    if (eoi_marker != 0xFFD9 && eoi_marker != 0xD900 && eoi_marker != 0x0000) {
      US_LOG_DEBUG(
          "Discarding truncated JPEG frame: eoi_marker=0x%04x, bytesused=%u",
          eoi_marker, buf->bytesused);
      return false;
    }
  }
...
static int _device_apply_dv_timings(us_device_s *dev) {
  ...
  if (_D_XIOCTL(VIDIOC_QUERY_DV_TIMINGS, &dv) == 0) {
    if (dv.type == V4L2_DV_BT_656_1120) {
      ...
      const unsigned vtot =
          V4L2_DV_BT_FRAME_HEIGHT(&dv.bt) / (dv.bt.interlaced ? 2 : 1);
      const unsigned fps =
          ((htot * vtot) > 0
               ? ((100 * (uint64_t)dv.bt.pixelclock)) / (htot * vtot)
               : 0);
      US_LOG_INFO(
          "Got new DV-timings: %ux%u%s%u.%02u, pixclk=%llu, vsync=%u, hsync=%u",
          dv.bt.width, dv.bt.height, (dv.bt.interlaced ? "i" : "p"), fps / 100,
          fps % 100, (unsigned long long)dv.bt.pixelclock, dv.bt.vsync,
          dv.bt.hsync); // See #11 about %llu
    } else {
      US_LOG_INFO("Got new DV-timings: %ux%u, pixclk=%llu, vsync=%u, hsync=%u",
                  dv.bt.width, dv.bt.height,
                  (unsigned long long)dv.bt.pixelclock, dv.bt.vsync,
                  dv.bt.hsync);
    }
    ...
    if (_D_XIOCTL(VIDIOC_QUERYSTD, &dev->standard) == 0) {
      US_LOG_INFO("Applying the new VIDIOC_S_STD: %s ...",
                  _standard_to_string(dev->standard));
      if (_D_XIOCTL(VIDIOC_S_STD, &dev->standard) < 0) {
...
static int _device_open_format(us_device_s *dev, bool first) { // FIXME
  ...
  US_LOG_DEBUG("Probing device format=%s, stride=%u, resolution=%ux%u ...",
               _format_to_string_supported(dev->format), stride, _RUN(width),
               _RUN(height));
...
#define FMTS(x_next)                                                           \
  (_D_IS_MPLANE ? fmt.fmt.pix_mp.plane_fmt[0].x_next : fmt.fmt.pix.x_next)

  // Check resolution
  bool retry = false;
  if (FMT(width) != _RUN(width) || FMT(height) != _RUN(height)) {
    US_LOG_ERROR("Requested resolution=%ux%u is unavailable", _RUN(width),
                 _RUN(height));
    retry = true;
  }
  ...
  if (FMT(pixelformat) != dev->format) {
    ...
    char *format_str;
    if ((format_str = (char *)_format_to_string_nullable(FMT(pixelformat))) !=
        NULL) {
...
#define SETFPS_TPF(x_next) setfps.parm.capture.timeperframe.x_next
  ...
  if (SETFPS_TPF(denominator) ==
      0) { // Не знаю, бывает ли так, но пускай на всякий случай
  ...
  if (dev->desired_fps != _RUN(hw_fps)) {
    US_LOG_INFO("Using HW FPS: %u -> %u (coerced)", dev->desired_fps,
                _RUN(hw_fps));
  } else {
...
static void _device_open_jpeg_quality(us_device_s *dev) {
  ...
  if (us_is_jpeg(_RUN(format))) {
    ...
    if (_D_XIOCTL(VIDIOC_G_JPEGCOMP, &comp) < 0) {
      US_LOG_ERROR(
          "Device doesn't support setting of HW encoding quality parameters");
    } else {
      comp.quality = dev->jpeg_quality;
      if (_D_XIOCTL(VIDIOC_S_JPEGCOMP, &comp) < 0) {
        US_LOG_ERROR("Can't change MJPEG quality for JPEG source with HW "
...
static int _device_open_io_method(us_device_s *dev) {
  US_LOG_INFO("Using IO method: %s",
              _io_method_to_string_supported(dev->io_method));
  switch (dev->io_method) {
  case V4L2_MEMORY_MMAP:
    return _device_open_io_method_mmap(dev);
  case V4L2_MEMORY_USERPTR:
    return _device_open_io_method_userptr(dev);
  default:
    assert(0 && "Unsupported IO method");
  }
  return -1;
}
...
static int _device_open_io_method_mmap(us_device_s *dev) {
  ...
  for (_RUN(n_bufs) = 0; _RUN(n_bufs) < req.count; ++_RUN(n_bufs)) {
    ...
    US_LOG_DEBUG("Calling us_xioctl(VIDIOC_QUERYBUF) for device buffer=%u ...",
                 _RUN(n_bufs));
...
#define HW(x_next) _RUN(hw_bufs)[_RUN(n_bufs)].x_next
    ...
    const size_t buf_size =
        (_D_IS_MPLANE ? buf.m.planes[0].length : buf.length);
    const off_t buf_offset =
        (_D_IS_MPLANE ? buf.m.planes[0].m.mem_offset : buf.m.offset);

    US_LOG_DEBUG("Mapping device buffer=%u ...", _RUN(n_bufs));
    if ((HW(raw.data) = mmap(NULL, buf_size, PROT_READ | PROT_WRITE, MAP_SHARED,
                             _RUN(fd), buf_offset)) == MAP_FAILED) {
...
static int _device_apply_resolution(us_device_s *dev, unsigned width,
                                    unsigned height) {
  // Тут VIDEO_MIN_* не используются из-за странностей минимального разрешения
  // при отсутствии сигнала у некоторых устройств, например TC358743
  if (width == 0 || width > US_VIDEO_MAX_WIDTH || height == 0 ||
      height > US_VIDEO_MAX_HEIGHT) {
...
#define SET_CID_DEFAULT(x_cid, x_field, x_quiet)                               \
  {                                                                            \
    struct v4l2_queryctrl m_query;                                             \
    if (_device_query_control(dev, &m_query, #x_field, x_cid, x_quiet) == 0) { \
      _device_set_control(dev, &m_query, #x_field, x_cid,                      \
                          m_query.default_value, x_quiet);                     \
    }                                                                          \
  }
...
#define CONTROL_AUTO_CID(x_cid_auto, x_cid_manual, x_field)                    \
  {                                                                            \
    if (dev->ctl.x_field.mode == CTL_MODE_VALUE) {                             \
      ...
      SET_CID_VALUE(x_cid_auto, x_field##_auto, 0,                             \
                    true); /* Reset inactive flag */                           \
  ...
  CONTROL_AUTO_CID(V4L2_CID_AUTO_WHITE_BALANCE,
                   V4L2_CID_WHITE_BALANCE_TEMPERATURE, white_balance);
...
static int _device_query_control(us_device_s *dev, struct v4l2_queryctrl *query,
  ...
  if (_D_XIOCTL(VIDIOC_QUERYCTRL, query) < 0 ||
...
static void _device_set_control(us_device_s *dev,
  ...
  if (value < query->minimum || value > query->maximum ||
    ...
    if (!quiet) {
      US_LOG_ERROR(
          "Invalid value %d of control %s: min=%d, max=%d, default=%d, step=%u",
          value, name, query->minimum, query->maximum, query->default_value,
          query->step);
    }
    return;
  }
