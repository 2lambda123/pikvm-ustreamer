...
static void _jpeg_write_scanlines_yvyu(struct jpeg_compress_struct *jpeg,
                                       const us_frame_s *frame);
...
static void _jpeg_write_scanlines_rgb565(struct jpeg_compress_struct *jpeg,
                                         const us_frame_s *frame);
static void _jpeg_write_scanlines_rgb24(struct jpeg_compress_struct *jpeg,
                                        const us_frame_s *frame);
static void _jpeg_write_scanlines_bgr24(struct jpeg_compress_struct *jpeg,
                                        const us_frame_s *frame);
...
void us_cpu_encoder_compress(const us_frame_s *src, us_frame_s *dest,
                             unsigned quality) {
  ...
  jpeg.in_color_space =
      ((src->format == V4L2_PIX_FMT_YUYV || src->format == V4L2_PIX_FMT_UYVY)
           ? JCS_YCbCr
           : JCS_RGB);
...
#define WRITE_SCANLINES(x_format, x_func)                                      \
  ...
  default:
    assert(0 && "Unsupported input format for CPU encoder");
  }
  ...
  if (jpeg->dest == NULL) {
    assert(
        (jpeg->dest = (struct jpeg_destination_mgr *)(*jpeg->mem->alloc_small)(
             (j_common_ptr)jpeg, JPOOL_PERMANENT,
             sizeof(_jpeg_dest_manager_s))) != NULL);
  }
...
static void _jpeg_write_scanlines_uyvy(struct jpeg_compress_struct *jpeg,
  ...
  while (jpeg->next_scanline < frame->height) {
    ...
    for (unsigned x = 0; x < frame->width; ++x) {
      // See also:
      // https://www.kernel.org/doc/html/v4.8/media/uapi/v4l/pixfmt-uyvy.html
      const bool is_odd_pixel = x & 1;
...
static void _jpeg_init_destination(j_compress_ptr jpeg) {
  ...
  assert((dest->buf = (JOCTET *)(*jpeg->mem->alloc_small)(
              (j_common_ptr)jpeg, JPOOL_IMAGE,
              JPEG_OUTPUT_BUFFER_SIZE * sizeof(JOCTET))) != NULL);
